...
{%- capture recovery_update %}In some cases, a newer Kraken version may not install due to an outdated recovery.
...
The updater app does not support upgrades from one version of Kraken to another, and will block installation to any update for a different version. Upgrading manually requires similar steps to installing Kraken for the first time.

1. Download the [Kraken install package](https://download.aospk.org/{{ device.codename }}) that you'd like to install or [build]({{ "devices/" | append: device.codename | append: "/build" | relative_url }}) the package yourself.
...
7. Sideload the Kraken `.zip` package:
...
If you have any questions or get stuck on any of the steps, feel free to ask on [our Telegram group](https://t.me/AOSPKChat).
