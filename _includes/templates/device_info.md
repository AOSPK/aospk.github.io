...
## Get Kraken for the {{ device.vendor }} {{ device.name }}
[Get the builds here](https://download.aospk.org/{{ device.codename }})
...
{%- endif %}
- [Update to a newer build of the same Kraken version]({{ "devices/" | append: device.codename | append: "/update" | relative_url }})
