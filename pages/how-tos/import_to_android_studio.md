...
Open a shell and navigate to the root folder of your Android sources, e.g. `~/android/kraken`.
...
In case you are developing on multiple branches or plan to do so (otherwise skip the next step), do the following (replace every occurrence of `kraken` with your desired naming scheme):

```
mv android.ipr kraken.ipr
mv android.iml kraken.iml && ln -s kraken.iml android.iml
```

## Prepare Android Studio

Android Studio struggles to parse the huge Kraken / Android sources and requires some different settings than the defaults. Therefore, before starting actual development, it is necessary to change some of those.
...
After you have modified the sources and tested your change, you can contribute to Kraken by [submitting your work]({{ "help/submit-patch/" | relative_url}}).
